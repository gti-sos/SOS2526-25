<script>
    ...
    let searchParams = $state({ country: "", year: "", air_arrival: "", water_arrival: "", land_arrival: "" });
    
    let message = $state(""); 

    const API_URL = "/api/v1/international-tourist-arrivals";
    ...
    async function searchArrivals() {
        ...
        if (searchParams.air_arrival) { query.append("air_arrival", searchParams.air_arrival); filtrosUsados.push(`llegadas aire '${searchParams.air_arrival}'`); }
        if (searchParams.water_arrival) { query.append("water_arrival", searchParams.water_arrival); }
        if (searchParams.land_arrival) { query.append("land_arrival", searchParams.land_arrival); }
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Cumplimos la rúbrica
                let textoFiltros = filtrosUsados.length > 0 ? filtrosUsados.join(', ') : "esos parámetros";
                message = `⚠️ No existe un registro de Llegadas Internacionales con ${textoFiltros}.`;
                arrivals = [];
    ...
    async function clearSearch() {
        searchParams = { country: "", year: "", air_arrival: "", water_arrival: "", land_arrival: "" };
        await getArrivals();
    ...
    async function addArrival() {
        const dataToSend = {
            ...
            air_arrival: parseFloat(newEntry.air_arrival),
            water_arrival: parseFloat(newEntry.water_arrival),
            land_arrival: parseFloat(newEntry.land_arrival)
        };
...
<main>
    ...
    <div class="card search-container">
        ...
        <form on:submit|preventDefault={searchArrivals}>
            <div class="input-group">
                ...
                <input type="number" step="0.1" placeholder="Llegadas Aire" bind:value={searchParams.air_arrival}>
                <input type="number" step="0.1" placeholder="Llegadas Mar" bind:value={searchParams.water_arrival}>
                <input type="number" step="0.1" placeholder="Llegadas Tierra" bind:value={searchParams.land_arrival}>
    ...
    <div class="card form-container">
        ...
        <form on:submit|preventDefault={addArrival}>
            <div class="input-group">
                ...
                <input type="number" step="0.1" placeholder="Llegadas Aire" bind:value={newEntry.air_arrival} required>
                <input type="number" step="0.1" placeholder="Llegadas Mar" bind:value={newEntry.water_arrival} required>
                <input type="number" step="0.1" placeholder="Llegadas Tierra" bind:value={newEntry.land_arrival} required>
    ...
    <div class="card table-container">
        <table>
            <thead>
                <tr>
                    <th>País</th><th>Año</th><th>Aire</th><th>Mar</th><th>Tierra</th><th>Acciones</th>
                </tr>
            </thead>
            <tbody>
                ...
                {#each arrivals as arrival}
                    <tr>
                        <td>{arrival.country}</td><td>{arrival.year}</td>
                        <td>{arrival.air_arrival}</td><td>{arrival.water_arrival}</td><td>{arrival.land_arrival}</td>
                        <td>
                            <button class="btn-edit" on:click={() => goto(`/international-tourist-arrivals/${arrival.country}/${arrival.year}`)}>Editar</button>
                            <button class="btn-delete" on:click={() => deleteArrival(arrival.country, arrival.year)}>Borrar</button>
                        </td>
                    </tr>
                {/each}
            </tbody>
