<script>
    ...
    async function loadAndDraw() {
        try {
            // 1. Fetch a la API de Temperaturas
            ...
            // Si la API del compañero está vacía, cargamos iniciales
            if (dataEsg.length === 0) {
            ...
            // Quitamos el mensaje, lo que hace que el div contenedor ya no sea 'hidden'
            message = ""; 

            // 4. EL TRUCO: Esperar a que el navegador expanda el div antes de dibujar
            setTimeout(() => {
                ...
                options = {
                    ...
                    titleTextStyle: { color: '#ffffff', fontSize: 20, bold: true },
                    backgroundColor: 'transparent',
                    
                    // Ahora la gráfica ocupará el 85% de los 750px que le hemos dado al div
                    chartArea: { width: '85%', height: '70%', top: 70, bottom: 80 }, 
                    
                    hAxis: { 
                        title: 'Año', 
                        textStyle: { color: '#cbd5e1', fontSize: 13, bold: true },
                    ...
                    vAxes: {
                        0: { 
                            ...
                            textStyle: { color: '#00f2fe', bold: true, fontSize: 13 },
                        ...
                        1: { 
                            title: 'Puntos ESG', 
                            textStyle: { color: '#10b981', bold: true, fontSize: 13 },
                    ...
                    series: {
                        0: { targetAxisIndex: 0, color: '#00f2fe' }, 
                        1: { targetAxisIndex: 1, color: '#10b981' }  
                    },
                    legend: { 
                        position: 'bottom', 
                        alignment: 'center',
                        textStyle: { color: '#ffffff', fontSize: 15, bold: true }
                    },
                    isStacked: false,
                    areaOpacity: 0.3, 
                    lineWidth: 4,
                    tooltip: { 
                        isHtml: true, 
                        textStyle: { color: '#0f172a', fontSize: 15 } 
                    }
                };

                chart = new window.google.visualization.SteppedAreaChart(chartContainer);
                chart.draw(dataTable, options);
            }, 150); // <-- Los 150 milisegundos de gracia para Svelte
...
<main>
    ...
    <div class="card" class:hidden={!!message}>
        <div bind:this={chartContainer} style="width: 100%; height: 750px;"></div>
    </div>
</main>
...
<style>
    ...
    h2 { color: #10b981; text-align: center; margin-bottom: 0.5rem; font-size: 2rem; }
    .subtitle { text-align: center; color: #94a3b8; margin-bottom: 2rem; font-size: 1.1rem; }
    .back-btn { color: #94a3b8; text-decoration: none; font-weight: bold; display: inline-block; margin-bottom: 1rem; transition: 0.2s;}
    .back-btn:hover { color: #10b981; }
    .card { background: rgba(255, 255, 255, 0.03); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 15px; padding: 2rem 1.5rem; box-shadow: 0 4px 20px rgba(0,0,0,0.3); }
    ...
    :global(.google-visualization-tooltip-item span) {
